...
# patch("api.routes.chat.stream_chat", ...) 가 경로를 해석하려면 해당 서브모듈이
# 미리 로드돼 있어야 한다(그렇지 않으면 AttributeError: module 'api.routes' has
# no attribute 'chat'). 모듈 레벨에서 한 번 import 해 등록해 둔다.
