...
/**
 * 외부 URL 에서 콘텐츠를 가져오는 fetcher 인터페이스.
 *
 * <p>네트워크 레이어와 파싱 레이어를 분리한다.
 * 실제 구현체({@link WebClientFeedHtmlFetcher})는 RestClient 를 사용하고,
 * 테스트 대역(stub/mock)은 픽스처를 반환해 네트워크 없이 파서를 검증할 수 있다.
 *
 * <p>모든 메서드 반환값이 null 이면 호출자에서 skip &amp; log 처리한다.
 */
public interface FeedHtmlFetcher {
    ...
    /**
     * 주어진 URL 의 응답 바이트를 반환한다 (PDF 등 바이너리 리소스 다운로드용).
     *
     * @param url 다운로드 대상 URL
     * @return 바이트 배열. 오류 시 null 반환 (예외 전파 금지 — skip &amp; log 처리)
     */
