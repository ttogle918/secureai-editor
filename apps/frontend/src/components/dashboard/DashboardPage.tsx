...
import { AutomationQuickActions } from '@/components/dashboard/AutomationQuickActions';
    ...
    <div style={{ flex: 1, overflowY: 'auto', background: 'var(--bg-0)', display: 'flex', flexDirection: 'column' }}>
      ...
      >
        ...
        <div className="kpi-grid dashboard-kpi-grid" aria-label="주요 보안 지표">
          ...
          <KpiCard
            value={localTotal > 0 ? `${Math.round((patched / localTotal) * 100)}%` : '—'}
            label="패치 완료율"
            color="var(--low)"
          />
        </div>

        {/* ── 정보 카드 3열 (크레딧 · 컴플라이언스 · 도메인 모니터링) ── */}
        <DashboardInfoCards
        ...
        {/* ── 자동화 빠른 액션 ── */}
        <AutomationQuickActions projectId={projectId} />
        ...
        {/* ── 토큰 사용량 ── */}
        {lastTokenUsage && <TokenUsageCard usage={lastTokenUsage} />}
