# SecureAI — ERD 설계서 (Entity Relationship Design) v3.0
> 작성자: 데이터 아키텍처 전문가 / 시니어 백엔드 개발자 공동 작성  
> 기준 DB: PostgreSQL 15 | ORM: Spring Data JPA (Hibernate 6)  
> 작성일: 2026-05-22 | 버전: v3.0 (Sprint 10 Enterprise 기능 고도화 반영)  
> **정본 지정일**: 2026-05-23 | 구버전(`01_ERD.md`, `01_ERD_V2.md`) 아카이브 완료

## 버전 이력

| 버전 | 기준일 | 주요 변경 |
|------|--------|---------|
| V1 | 2026-04-19 | 초기 설계 — plans, users, refresh_tokens, projects, team_members, analysis_sessions, vulnerabilities, patch_suggestions, cve_data (9개 테이블) |
| V2 | 2026-04 (Sprint 5) | PR_REVIEW_HISTORY, SCAN_TARGETS, GITHUB_CONFIGS 추가. 취약점 DAST 연계 컬럼, vuln_fingerprint 중복 탐지 추가 |
| **V3 (현재)** | 2026-05-22 | Sprint 8/9: MONITORING_RESULTS(월별 파티션), TEAM_SETTINGS, SECURITY_DOC_REQUESTS, SHEDLOCK, AUDIT_LOGS(파티션), TOTP 필드 추가. Sprint 10: PROJECT_SCHEDULES, Enterprise 컬럼(scan_mode, roi_summary) 보강. 총 22개 테이블 |
...
## 0. ⚠️ 현재 스키마 정합 (2026-06-29 정정 — V045~V064, 본 섹션 우선)

> 본문(§1~§2)은 **Sprint 10(V044)** 기준. 실제 Flyway는 **V064**까지 진행됐고 아래 테이블/컬럼이 본문에 **없다**. 충돌 시 본 §0이 정본.

### 0.1 신규 테이블 (Sprint 11~14)

| 테이블 | 마이그레이션 | 핵심 컬럼 | 용도 |
|--------|------------|-----------|------|
| `user_provider_keys` | V052 | user_id, provider(anthropic/gemini/openai), encrypted_key, default_model | BYOK 멀티프로바이더 키(AES) |
| `token_usage` | V054 | session_id, user_id, project_id, provider, model, input/output/cache_creation/cache_read_tokens, cost_usd, occurred_at | 토큰·원가 추적(TASK-1204, ECON) |
| `user_sessions` | V056 | user_id, jwt_jti, device_info, ip, user_agent, revoked_at, expires_at | 세션 이력·강제 로그아웃(TASK-1202b) |
| `email_log` | V057 | to_address, subject, status(SENT/FAILED/SUPPRESSED), provider, attempts | 트랜잭션 이메일(TASK-1210) |
| `email_suppression` | V058 | email_address, reason(BOUNCE/COMPLAINT) | 발송 억제 목록 |
| `triage_feedback` | V060 | vulnerability_id, user_id, action(CONFIRM/DISMISS/ACCEPT_PATCH), reason | 트리아지 학습데이터(MOAT-1, append-only) |
| `compliance_frameworks` / `compliance_controls` | V062 | doc_type, version, official/form_link / control_id, category, related_vuln_types(JSONB) | 컴플라이언스 프레임워크·통제 매핑 |

### 0.2 신규 컬럼 (기존 테이블 ALTER)

| 테이블 | 추가 컬럼 | 마이그레이션 |
|--------|-----------|------------|
| `users` | security_score(V045), workspace_mode(V048), terms_accepted_at·privacy_accepted_at·marketing_opt_in(V049), preferred_provider(V053) | V045/48/49/53 |
| `pr_review_history` | project_id NULL 허용(V050), session_id·installation_id(V051) | V050/51 |
| `audit_logs` | prev_hash·current_hash(해시체이닝) | V055 |
| `analysis_sessions` | status → VARCHAR(50)(AWAITING_CONFIRMATION 등 수용) | V059 |
| `patch_suggestions` | verification_status·verified_at·test_code·verification_log | V061 |
| `security_doc_requests` | framework_version | V063 |
| (data) `security_guidelines` | URL/API 노출 가이드라인 행 추가 | V064 |

> §6 Flyway 목록(V041~ "예정")도 stale — 실제 V044~V064 모두 적용됨.
...
└───────────────────┘   └──────────────────────┘   └─────────────────────┘
┌───────────────────┐   ┌──────────────────────┐   ┌─────────────────────┐
│   TEAM_SETTINGS   │   │ SECURITY_DOC_REQUESTS │   │      SHEDLOCK       │
│  (IP Allowlist)   │   │  (보안 문서 이력)     │   │   (분산 락)         │
└───────────────────┘   └──────────────────────┘   └─────────────────────┘
┌───────────────────┐
│ PROJECT_SCHEDULES │
│ (야간 스캔 설정)  │
...
CREATE TABLE users (
    ...
    security_score        INTEGER         NOT NULL DEFAULT 0,   -- 게이미피케이션 점수
    workspace_mode        VARCHAR(20)     NOT NULL DEFAULT 'DEVELOPER',
    ...
    -- 2FA (TOTP, Sprint 8 추가)
    totp_enabled          BOOLEAN         NOT NULL DEFAULT FALSE,
    totp_secret           VARCHAR(64),
    totp_recovery_codes   TEXT[],                               -- 해시 저장
...
CREATE TABLE analysis_sessions (
    ...
    scan_mode         VARCHAR(20)     NOT NULL DEFAULT 'audit',-- 'audit' (종합), 'pipeline' (선별)
...
CREATE TABLE vulnerabilities (
    ...
    time_to_remediate_seconds INTEGER,                       -- MTTR 측정용
...
CREATE TABLE reports (
    ...
    roi_summary       JSONB,                                   -- {time_saved_hours, cost_saved_usd} (Enterprise B2B)
...
### 2.19 TEAM_SETTINGS (Sprint 8)

```sql
CREATE TABLE team_settings (
    id                UUID        PRIMARY KEY DEFAULT gen_random_uuid(),
    project_id        UUID        NOT NULL UNIQUE REFERENCES projects(id) ON DELETE CASCADE,
    ip_allowlist      TEXT[],                                 -- CIDR 목록
    created_at        TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at        TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
```
...
### 2.20 SECURITY_DOC_REQUESTS (Sprint 8)
...
CREATE TABLE security_doc_requests (
    ...
    doc_type          VARCHAR(50) NOT NULL,                   -- 'ISMS_P', 'CISO'
...
### 2.22 PROJECT_SCHEDULES (Sprint 10)
...
CREATE TABLE project_schedules (
    ...
    cron_expression   VARCHAR(50)     NOT NULL DEFAULT '0 0 * * *', -- 매일 자정
    target_branch     VARCHAR(100)    NOT NULL DEFAULT 'main',
    scan_mode         VARCHAR(20)     NOT NULL DEFAULT 'audit',
    only_modified     BOOLEAN         NOT NULL DEFAULT TRUE,        -- Git 커밋/수정 날짜 기준 변경 파일만 검사
...
CREATE INDEX idx_project_schedules_next_run ON project_schedules(next_run_at) WHERE is_active = TRUE;
```
...
-- Sprint 8 --
V037__create_shedlock_table.sql
V038__add_totp_fields.sql
V039__create_team_settings.sql
V040__create_security_doc_requests.sql

-- Sprint 9 이후 (순차 배정 예정) --
V041~ : monitoring_results_partitioned, indexes, triggers
```
