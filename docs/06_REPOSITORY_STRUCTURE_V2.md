...
│
├── backend/                                          # Spring Boot 3 백엔드 서비스 (섹션 2)
├── ai-agent/                                         # Python FastAPI + LangGraph AI 에이전트 (섹션 3)
├── mcp-server/                                       # MCP Server Node.js (섹션 4)
├── frontend/                                         # Next.js 15 웹 클라이언트 (섹션 5)
├── android/                                          # Kotlin Android 앱 (섹션 6)
├── dast-sandbox/                                     # Python DAST 격리 컨테이너 (섹션 7)
├── infra/                                            # 인프라 설정 (섹션 8)
├── docs/                                             # 설계 문서 (섹션 9)
│
...
└── src/
    ...
    │   │   │   │   ├── controller/
    │   │   │   │   │   └── AnalysisController.java   # POST /sessions (분석 시작), GET /sessions/{id}/stream (SSE), GET /sessions/{id}
    │   │   │   │   ├── service/
    │   │   │   │   │   ├── AnalysisService.java      # 세션 생성·조회, 플랜 사용량 체크, AI Agent 호출 오케스트레이션
    │   │   │   │   │   ├── AiAgentClient.java        # ⭐ Python AI Agent HTTP 클라이언트 (Resilience4j Circuit Breaker 적용)
    │   │   │   │   │   └── SseEmitterService.java    # Redis SUBSCRIBE → SseEmitter 브릿지, 채널 생명주기 관리
    │   │   │   │   ├── dto/
    │   │   │   │   │   ├── AnalysisStartRequest.java # projectId, layerType, targetPath, options
    │   │   │   │   │   ├── AnalysisStartResponse.java # sessionId, sseUrl (즉시 202 반환)
    │   │   │   │   │   ├── AnalysisSessionResponse.java # 세션 전체 상태 + 집계 결과
    │   │   │   │   │   ├── AgentAnalyzeRequest.java  # AI Agent로 전달하는 요청 페이로드
    │   │   │   │   │   └── SseEvent.java             # SSE 이벤트 종류별 공통 페이로드 DTO
    │   │   │   │   ├── entity/
    │   │   │   │   │   └── AnalysisSession.java      # analysis_sessions 테이블 엔티티 (집계 컬럼, 파티션 테이블)
    │   │   │   │   ├── repository/
    │   │   │   │   │   └── AnalysisSessionRepository.java # 프로젝트별·상태별 세션 조회
    │   │   │   │   └── event/
    │   │   │   │       ├── SessionCompletedEvent.java # 세션 완료 이벤트 → 프로젝트 점수 갱신, FCM Push 트리거
    │   │   │   │       └── SessionFailedEvent.java   # 세션 실패 이벤트 → 사용량 롤백, 실패 알림
    │   │   │   │
    │   │   │   ├── vulnerability/                    # 취약점 도메인
    │   │   │   │   ├── controller/
    │   │   │   │   │   └── VulnerabilityController.java # GET /vulns (복합 필터), PATCH /status, GET /api-groups
    │   │   │   │   ├── service/
    │   │   │   │   │   ├── VulnerabilityService.java # 취약점 저장·조회(필터+페이징), 상태변경, 지문 중복 체크
    │   │   │   │   │   └── VulnerabilityQueryService.java # CQRS Read 전담: 대시보드 집계 쿼리, 트렌드 계산
    │   │   │   │   ├── dto/
    │   │   │   │   │   ├── VulnerabilityResponse.java # 취약점 + callChain JSON + 패치/익스플로잇 존재 여부
    │   │   │   │   │   ├── VulnListRequest.java      # severity[], apiGroup, status, 페이징 파라미터
    │   │   │   │   │   ├── VulnStatusUpdateRequest.java # 새 status + reason (false_positive 사유 등)
    │   │   │   │   │   └── VulnTrendResponse.java    # 날짜별 {score, vulnTotal} 배열 (차트용)
    │   │   │   │   ├── entity/
    │   │   │   │   │   └── Vulnerability.java        # vulnerabilities 테이블 엔티티 (callChain JSONB, 지문 컬럼)
    │   │   │   │   ├── repository/
    │   │   │   │   │   └── VulnerabilityRepository.java # 세션+심각도+apiGroup 복합 필터 쿼리 (@Query)
    │   │   │   │   ├── listener/
    │   │   │   │   │   └── VulnerabilityEventListener.java # VulnerabilityFoundEvent 수신 → 세션 집계 업데이트
    │   │   │   │   └── event/
    │   │   │   │       └── VulnerabilityFoundEvent.java # AI Agent가 취약점 발견 시 발행 → SSE + 집계 업데이트
    │   │   │   │
    │   │   │   ├── dast/                             # DAST 동적 분석 도메인
    │   │   │   │   ├── controller/
