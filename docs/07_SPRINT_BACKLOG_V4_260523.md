...
#### FEAT-COMP-005 🟠 — 컴플라이언스 피드 실데이터화 (KISA RAG 트랙의 토대)
...
| FEAT-FE-010 | PR 경로 정합화 — git 루트 기준 파일 경로 | 🟠 High | **문제**: 로컬 폴더 스캔(`useWorkspace.readDir`)은 파일 경로를 **고른 폴더 기준 상대경로**로 기록(`apps/frontend`를 열면 → `src/foo.ts`, `apps/frontend/` 프리픽스 소실). 이 경로로 PR 생성 시 대상 레포 **루트의 `src/foo.ts`에 커밋**돼 in-place 업데이트가 안 됨(또는 `PATCH_CONTENT_UNAVAILABLE`). **제약**: 브라우저 `showDirectoryPicker`는 부모 디렉토리·`.git`·git 명령 접근 불가 → "`../`로 git remote 탐색"은 **웹 불가, Tauri(데스크톱)만 가능**. **해법**: ①(웹) PR 모달에 **경로 프리픽스 입력** 또는 "레포 루트를 열어주세요" 가이드 + 대상 URL·커밋경로 미리보기(2026-06-28 모달 미리보기 추가됨) ②(웹 권장) **GitHub 스캔** 경로는 이미 레포 루트 기준이라 PR 정확 — PR 데모는 GitHub 스캔 플로우로 ③(Tauri) `git rev-parse --show-toplevel`로 루트 탐색해 경로 자동 보정. ④**특정 디렉토리만 스캔하되 상위 폴더에 `.git`이 있는 경우 대비**: (Tauri) 연 폴더에서 위로 올라가며 `.git` 발견 시 그 폴더를 루트로 삼고 경로를 루트 기준으로 보정 + 스캔 범위는 연 디렉토리로 한정. (웹) `.git`을 못 보므로 "스캔할 디렉토리 + 레포 내 경로 프리픽스"를 사용자가 지정하는 옵션 제공. **데모 임시조치(2026-06-28)**: kkebi를 clone해 `demo-vuln-sample/`를 넣고, 에디터에서 **레포 루트(kkebi 폴더)를 열어** 스캔→경로가 `demo-vuln-sample/app.py`로 잡혀 PR 정상. **검증 필요**: GitHub 스캔→clone→스캔→PR 왕복 실경로 테스트(미수행). |
| FEAT-COMP-005 | 컴플라이언스 페이지 실데이터화 (보안정보 큐레이션) | 🟠 High | `/compliance` 페이지를 mock→실데이터로. **2026-06-28 mock 4섹션 구현**(보안점검 체크리스트·정부권장·최신뉴스·규제프레임워크). **단계별 실현**: **A. 요약+링크(시작점)** — 백엔드 크롤러 엔드포인트 + 스케줄 잡(예: 1일 1회)이 KISA 보호나라·행안부·보안뉴스 등 **게시물을 전부 수집**(KISA 주요 게시물은 수가 적음) → 제목·날짜·**실제 카테고리 태그**·요약을 **DB 저장** → FE는 캐시에서 즉시 로드, **파일 다운로드는 원문 링크로 유도**(CORS·저작권 회피). **B. 파일 캐시** — 게시물 첨부 PDF를 서버가 미리 받아 저장(갱신·스토리지 관리). **C. AI 요약** — PDF 파싱→LLM 요약(파일 내용까지 반영한 요약), 오프라인 배치(토큰비용 고려). **레이아웃**: 자체 큐레이션·요약(보안 정보 전체 내용)을 **상단 잘 보이는 곳**, 기관별 주요 게시물은 **기관당 하단에 스크롤로 목록화**. **제약**: 외부 콘텐츠는 요약만(원문 복제 금지), 브라우저 직접 fetch 불가(백엔드 프록시 필수). 관련: FEAT-RPT-001(야간 다이제스트)와 데이터소스 공유 가능. |
| FIX-PATCH-001 | 패치 적용 시 들여쓰기 보존 (PR 코드 품질) | 🟠 High | **버그**: PATCH→PR 생성 시 패치된 코드 첫 줄이 **들여쓰기를 잃고 column 0**에 붙음. 예) `demo-vuln-sample/app.py` SSRF 패치에서 `+    url = request.args.get("url")` 가 `+url = request.args.get("url")` 로 들어가 함수 본문 들여쓰기가 깨짐 → **실행 시 Python IndentationError**. diff 표시는 정상으로 보이나 실제 코드가 망가짐. **원인 후보**: ai_engine `diff_generator`의 스니펫 치환 또는 Backend `PatchPrService`의 원본파일 취약구간 치환 시 원본 indentation을 보존하지 않음. **할 일**: 치환 시 취약 라인의 선행 공백(indent)을 감지해 패치 코드 각 줄에 동일 적용, 또는 LLM 패치를 원본 들여쓰기 기준으로 정규화. **검증**: PR diff가 문법적으로 유효한 Python/Java가 되는지(파싱 테스트). (2026-06-28 kkebi PR #1에서 발견) |
