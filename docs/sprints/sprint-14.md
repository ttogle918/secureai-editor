...
## 📅 2026-06-26 FE 데모 UI 구현 (병렬 진행)

**Context**: 직전 세션(2026-06-24) VAL-4·TASK-1401·1402 코드 완료 후, **데모 시연 준비용 FE UI** 구현. Claude Design 시안(IA Audit·Dashboard·Editor 리디자인)을 Next.js로 직접 구현.

### 완료 항목
- **`--bg-selected` 토큰**: 선택 행 배경 가시성 복구 (부트스트랩 이후 잠재버그)
- **벌크 트리아지 재이식**: Dev가 만든 기능이 렌더 안 되는 VulnPanel에 있음 발견 → 실제 패널 VulnDetailPanel로 이식 (멀티셀렉트·chevron 분리)
- **B: IA 사이드바 재편**: SECURITY/AUTOMATION/REPORTS 그룹화, 묻힌 기능(github-scan/sbom/compliance/admin/team) 진입점 가시화
- **C: 대시보드+헤더**: 크레딧 실데이터 연동, 자동화 빠른액션 버튼, useCredits 훅
- **D: 드래그-스플릿 에디터**: 2-pane 50/50 리사이저

### 의논 & 결정
1. **design-sync 전환**: design-sync 스킬 실패(Next.js 앱은 DS 라이브러리 아님) → 직접 구현으로 전환
2. **벌크 트리아지 오배치**: VulnPanel(미사용 레거시) vs VulnDetailPanel(실제 활동 컴포넌트) 명명 혼동 → 재이식 + UX 개선(행 클릭=멀티셀렉트, chevron=상세)
3. **정직성 원칙**: 가짜 데이터 금지 — 크레딧만 실데이터(`/users/me`), 컴플라이언스/모니터링은 "준비 중" 비활성
4. **에디터 추가기능(데모 후 백로그화)**: Ctrl+S·Git은 Tauri/웹 차등 → FEAT-FE-SAVE·FEAT-FE-GIT

### 데모 준비도 검증
| 항목 | 상태 | 비고 |
|------|------|------|
| **데모 시나리오 5개** | 4/5 완성 | Scene4(proven_scorecard CLI 벤치만, UI 없음)·Scene5(검증 자동, 수동 버튼 없음) 갭 있음 → 대체안 권장 |
| **환경 전제** | 부분 준비 | backend+AI+pg+redis 실행, WebGoat+dast-isolated-net(Scene4), secureai-patch-verify(Scene5), GitHub App 권한 필요 |
| **FE 검증** | ✅ 132 테스트 통과, typecheck/lint/build 클린 | 모든 커밋 Reviewer PASS |

### 이월
- **Sprint 15**: FEAT-FE-SAVE·FEAT-FE-GIT·VAL-18 UI 개선·FEAT-FE-AUTOMATION 상세·proven 프로덕션 UI·useCredits Context 확장

---

## 📅 2026-06-26 BYOK 검증·과금 2모드·빌링 페이지·분석 타이머

**Context**: VC 데모 시연 준비 — BYOK end-to-end 검증 + Gemini 폴백 버그 수정 + 과금 2모드(BYOK/PLATFORM) 명시 토글 구현 + 빌링 페이지 신규 + 분석 진행 경과 타이머.

### 완료 항목
- **Gemini BYOK 폴백 버그 수정** (`bdf80ba`, 머지 `44ca168`): 서버 GEMINI_API_KEY만 검사하던 우선순위 오류 → user_api_key를 폴백 위로 이동. SAST/DAST 모두 사용자 Gemini 키 정상 인식 (ai_engine 142 단위테스트 통과)
- **크레딧 충전 페이지 `/billing` 신규** (`cabd9e8`): 요금제 카드(Basic/Pro/Enterprise) + 사용량 그래프 + 헤더 크레딧칩 연결
- **FEAT-BILLING-2: 키모드 토글(BYOK/PLATFORM)** (`d3fd02f`): Backend `StartAnalysisRequest.keyMode` 필드 추가(enum BYOK/PLATFORM/null) + `AnalysisService`에서 PLATFORM 선택 시 BYOK 키 무시. Frontend localStorage(`keyMode`) + `/billing` 라디오 토글. null-safe로 기존 마이그레이션 불필요
- **분석 진행 경과 타이머** (`72b32f8`): FE `useAnalysisProgress.ts` + `AnalysisTimer.tsx` 경과시간 표시 (실 duration 백엔드 영속화는 FEAT-ANALYSIS-TIMING 백로그)

### 의논 & 결정
1. **BYOK end-to-end 검증**: FE 설정(3키 입력+프로바이더 선택) → 백엔드 `ProviderKeyService.resolveKeyForAnalysis` 프로바이더별 키 반환 → ai_engine `llm/factory.get_provider(provider, api_key)` 그 키로 호출. **Anthropic/OpenAI 정상, Gemini 폴백 버그 발견** — 서버 키 미설정 시 사용자 키 무시 → 조건 재정렬로 해소.
2. **기본 모델 확정**: PIPELINE(정밀)=`claude-sonnet-4-6`, AUDIT(빠름)=`claude-haiku-4-5`, 프로바이더=anthropic (사용자 선택 가능)
3. **과금 2모드 설계**: 모드1 BYOK(무제한·차감없음, `deductForScan` BYOK 제외) / 모드2 PLATFORM(플랫폼 키+크레딧 차감, TOKEN_LIMIT 한도). **FEAT-BILLING-2(명시 토글) 지금 구현** — 저위험 null-safe 설계로 기존 테스트 영향 0. ❌자가결제·⚠️API키발급 백로그
4. **FE-only 데모 우선**: 빌링 강화(요금제·사용량 그래프 더미)·타이머는 FE만. 실 백엔드 구조(duration 영속·그래프 실데이터) = FEAT-ANALYSIS-TIMING/FEAT-DASH-USAGE 백로그.
5. **VC 데모 보강**: Proven 배지·원가 투명성(타이머+토큰+그래프+모드토글)·탐지→증명→교정→PR 풀루프·벌크트리아지·규제증적·플랫폼폭(사이드바). **사전조건**: WebGoat+dast-isolated-net·패치검증이미지·GitHub App 권한·BYOK 테스트용 Anthropic키

### 형상관리
- 4커밋 push 완료 (Gemini 폴백 fix + 빌링 2모드 + 페이지 + 타이머)
- 사용자 산출물 `docs/design/2026-06-25-claude-design/` untracked 유지

### 다음 세션
- 데모 촬영 (SAST 노이즈→트리아지→Proven→자동PR→원가$/BYOK/크레딧 시나리오)
- 백로그: FEAT-BILLING-1(PG결제)·FEAT-BILLING-3(API키)·FEAT-ANALYSIS-TIMING·FEAT-DASH-USAGE·앞선 FE(SAVE/GIT)
