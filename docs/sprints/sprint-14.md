...
- **하위 할일(순서)**:
  - [ ] **(선행) `mapping.py` — WebGoat 엔드포인트↔SAST finding(CWE/vuln_type) 매핑 테이블 확정** (runner 블로킹 선행)
  - [ ] WebGoat compose(`dast-isolated-net`) + 헬스체크
  - [ ] runner.py — findings 루프 → executor 호출 → 집계
  - [ ] scorecard/csv 산출 + 데모 영상 녹화
...
| 마일스톤 | 기준 | 상태 |
|---------|------|------|
| proven_exploitable 증명 | WebGoat 대상 SQLi/XSS proven + scorecard + 데모영상 | ⬜ VAL-4 |
| 패치 자동 PR | 승인 패치 → 실제 PR 등록(PR-only) | ⬜ 1401 |
| 패치 자가 검증 | Python 패치 Docker 검증 → Verified/Failed 배지 | ⬜ 1402 |
