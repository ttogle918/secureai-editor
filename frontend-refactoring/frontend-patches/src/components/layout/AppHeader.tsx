...
export function AppHeader({ onExportJSON }: AppHeaderProps) {
  ...
  useSse({
    ...
    onEvent: (event) => {
      if (event.type === 'completed') {
        ...
        if (event.file) {
          addProgressStep({ stepName: event.node === 'sast' ? 'SAST 분석' : (event.node ?? '분석 중'), stepOrder: Date.now(), target: event.file, status: 'completed' });
        } else if (event.message) {
          addProgressStep({ stepName: event.node ?? '진행 중', stepOrder: Date.now(), target: event.message, status: 'completed' });
        }
      } else if (event.type === 'scan_complete') {
        addProgressStep({ stepName: '스캔 완료', stepOrder: Date.now(), target: `취약점 ${event.vuln_count ?? 0}개 발견`, status: 'completed' });
  ...
  const unreadNotifications = 0; // wired up when NotificationCenter is built
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 0 }}>
        ...
        >
          ...
          {!breadcrumb.length && (
            <span style={{ color: 'var(--text-tertiary)' }}>{viewMode === 'editor' ? '파일을 선택하세요' : 'Security Dashboard'}</span>
          )}
        </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        {/* Severity chips — uses new global .chip styles, brighter active text */}
          ...
          {SEV_FILTERS.map(sev => {
            ...
            return (
              ...
              >
                ...
                {sev.toUpperCase()} {sevCount(sev) > 0 && <span style={{ opacity: 0.75, marginLeft: 2 }}>{sevCount(sev)}</span>}
              </button>
            );
          })}
        </div>
